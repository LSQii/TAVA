...
# 就在 detection_collate 函数结束后，添加下面的代码
def custom_collate(batch):
    """
    自定义合并函数：过滤 batch 中的 None 样本（仅用于测试集）
    """
    # 1. 剔除所有 None 样本（加载失败的样本）
    batch = [x for x in batch if x is not None]
    # 2. 若过滤后 batch 为空，返回空 tuple（避免框架报错）
    if len(batch) == 0:
        return (torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]), {})
    # 3. 对有效样本，按原有逻辑合并（兼容普通测试、检测任务等）
    # 判断是否是检测任务（用 detection_collate），否则用默认合并
    if "boxes" in batch[0][4] or "ori_boxes" in batch[0][4]:
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        ...
        if (
            ...
            if cfg.DETECTION.ENABLE:
                ...
                # 测试集：使用自定义 collate 函数（过滤 None 样本）
                if split in ["test", "test_openset"]:
                    collate_func = custom_collate
                # 其他场景（训练/验证）：保持原有逻辑（用默认 default_collate）
                else:
                    collate_func = None
            loader = torch.utils.data.DataLoader(
                dataset,
