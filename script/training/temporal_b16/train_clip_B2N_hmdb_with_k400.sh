ROOT=/root/TAVA/
CKPT=/root/autodl-tmp 

# TRAIN_FILE can be set as train_1.csv or train_2.csv or train_3.csv;

B2N_hmdb_file=B2N_hmdb
TRAIN_FILE=train.csv
VAL_FILE=val.csv
TEST_FILE=test.csv

# ==========================================
# K400 Checkpoint 路径（如果有的话）
# ==========================================
# 取消注释并修改为实际的 K400 checkpoint 路径
# K400_CHECKPOINT=/root/autodl-tmp/basetraining/froster/wa_checkpoints/swa_2_22.pth
# 或者使用某个 epoch 的 checkpoint：
# K400_CHECKPOINT=/root/autodl-tmp/basetraining/froster/checkpoints/checkpoint_epoch_00022.pyth

cd $ROOT

TORCH_DISTRIBUTED_DEBUG=INFO python -W ignore -u tools/run_net.py \
  --cfg configs/Kinetics/TemporalCLIP_vitb16_8x16_STAdapter_HMDB51.yaml \
  --opts DATA.PATH_TO_DATA_DIR $ROOT/zs_label_db/$B2N_hmdb_file \
  DATA.PATH_PREFIX $ROOT/data/hmdb51/videos \
  TRAIN_FILE $TRAIN_FILE \
  VAL_FILE $VAL_FILE \
  TEST_FILE $TEST_FILE \
  DATA.PATH_LABEL_SEPARATOR , \
  DATA.INDEX_LABEL_MAPPING_FILE $ROOT/zs_label_db/B2N_hmdb/train_rephrased.json \
  DATA.PROXY_LABEL_MAPPING_FILE  $ROOT/zs_label_db/B2N_hmdb/test_rephrased.json \
  DATA.ACTION_ATTRIBUTE_MAPPING_FILE $ROOT/attribute_files/final_visual_attributes_hmdb.json \
  TRAIN.ENABLE True \
  OUTPUT_DIR $CKPT/basetraining/B2N_hmdb51_froster_from_k400 \
  TRAIN.BATCH_SIZE 32 \
  TEST.BATCH_SIZE 48 \
  TEST.NUM_ENSEMBLE_VIEWS 3 \
  TEST.NUM_SPATIAL_CROPS 1 \
  NUM_GPUS 4 \
  SOLVER.MAX_EPOCH 12 \
  SOLVER.WARMUP_EPOCHS 2.0 \
  SOLVER.BASE_LR 3.33e-6 \
  SOLVER.WARMUP_START_LR 3.33e-8 \
  SOLVER.COSINE_END_LR 3.33e-8 \
  TRAIN.MIXED_PRECISION True \
  DATA.DECODING_BACKEND "pyav" \
  MODEL.NUM_CLASSES 26 \
  MIXUP.ENABLE False \
  AUG.ENABLE False \
  AUG.NUM_SAMPLE 1 \
  TRAIN.EVAL_PERIOD 6 \
  TRAIN.CHECKPOINT_PERIOD 1 \
  MODEL.LOSS_FUNC soft_cross_entropy \
  TRAIN.LINEAR_CONNECT_CLIMB False \
  TRAIN.CLIP_ORI_PATH /root/.cache/clip/ViT-B-16.pt \
  TRAIN.LINEAR_CONNECT_LOSS_RATIO 0.0 \
  MODEL.RAW_MODEL_DISTILLATION True \
  MODEL.KEEP_RAW_MODEL True \
  MODEL.DISTILLATION_RATIO 2.0 \
  TRAIN.CHECKPOINT_FILE_PATH $K400_CHECKPOINT \
  TRAIN.CHECKPOINT_EPOCH_RESET True


  python weight_average_tool.py \
  --source_dir $CKPT/basetraining/B2N_hmdb51_froster_from_k400/checkpoints \
  --output_dir $CKPT/basetraining/B2N_hmdb51_froster_from_k400/wa_checkpoints \
  --raw_clip /root/.cache/clip/ViT-B-16.pt \
  --wa_start 2 \
  --wa_end 12

