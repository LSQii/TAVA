...
TORCH_DISTRIBUTED_DEBUG=INFO python -W ignore -u tools/run_net.py \
  ...
  DATA.PATH_PREFIX /root/autodl-tmp/UCF-101 \
  TRAIN_FILE $TRAIN_FILE \
  VAL_FILE $VAL_FILE \
  TEST_FILE $TEST_FILE \
  DATA.PATH_LABEL_SEPARATOR , \
  DATA.INDEX_LABEL_MAPPING_FILE $ROOT/zs_label_db/ucf101-index2cls.json \
  DATA.PROXY_LABEL_MAPPING_FILE  $ROOT/zs_label_db/B2N_ucf101/test_rephrased.json \
  DATA.ACTION_ATTRIBUTE_MAPPING_FILE $ROOT/attribute_files/final_visual_attributes_ucf.json \
  TRAIN.ENABLE True \
  OUTPUT_DIR $CKPT/basetraining/ucf101_froster \
  TRAIN.BATCH_SIZE 24 \
  TEST.BATCH_SIZE 24 \
...
if [ -d "$CKPT/basetraining/ucf101_froster/checkpoints" ]; then
  echo "训练完成，开始权重平均..."
  python weight_average_tool.py \
    --source_dir $CKPT/basetraining/ucf101_froster/checkpoints \
    --output_dir $CKPT/basetraining/ucf101_froster/wa_checkpoints \
