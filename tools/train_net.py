...
import sys
# 替换成你的「自定义 pytorchvideo」的父文件夹路径
CUSTOM_PATH = "root/TAVA/pytorchvideo-main" 
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                if cfg.MODEL.RECORD_ROUTING:
                    ...
                    #此处只需要读取proxy 类别的json文件，然后判断有多个类别就可以了。
                    proxy_class_mapping = json.load(open(os.path.join(cfg.DATA.PROXY_LABEL_MAPPING_FILE), 'r'))
                    # proxy_class_text_dict=model.text_prompt("/root/shared-nvme/FROSTER-main/zs_label_db/B2N_hmdb/test_rephrased.json")
                    # proxy_class_classfier=model.achieve_csf_matrix(proxy_class_text_dict,model.model,trainable=False)
                    # print(proxy_class_classfier.shape)
                    # torch.Size([25, 512])
                    # proxy_class_num , dim
                    bz=inputs[0].shape[0]
                    proxy_class_num=len(proxy_class_mapping)
                    
                    proxy_labels=random.sample(range(proxy_class_num), bz)
                    # print(proxy_labels)

                    if cfg.NUM_GPUS>1:
                        attribute_ids, attribute_similarities = model.module.retrieval_most_action_attributes(
                            proxy_labels,
                            os.path.join(cfg.DATA.PROXY_LABEL_MAPPING_FILE),
                            os.path.join(cfg.DATA.ACTION_ATTRIBUTE_MAPPING_FILE),
                            bz,
                            attribute_num_perInstance=4)

                        model.module.cur_epoch = cur_epoch
                    else:
                        attribute_ids, attribute_similarities = model.retrieval_most_action_attributes(
                            proxy_labels,
                            os.path.join(cfg.DATA.PROXY_LABEL_MAPPING_FILE),
                            os.path.join(cfg.DATA.ACTION_ATTRIBUTE_MAPPING_FILE),
                            bz)

                        model.cur_epoch = cur_epoch

                    


                    # print("train_net cur_epoch:")
                    # print(cur_epoch)
                    ...
                    outputs = model(inputs,attribute_ids, attribute_similarities)
                    if len(outputs[0]) == 2:
                        ...
                        [proxy_pred, preds, img_encode, text_encode], [_, raw_img_encode, raw_text_encode]=outputs
                    elif len(outputs[0]) == 5:
                        [proxy_pred, preds,similarity_distribution ,img_encode, text_encode], [_, raw_img_encode, raw_text_encode]=outputs
                else:
                    preds = model(inputs)
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss:
                ...
                if cur_epoch>1:
                    # print(proxy_pred.shape)
                    proxy_labels=torch.tensor(proxy_labels).to(proxy_pred.device)
                    proxy_pred_log_softmax = F.log_softmax(proxy_pred, dim=-1)
                    augmented_class_loss=loss_fun(proxy_pred, proxy_labels)
                    

                    # print(proxy_pred.device)
                    # print(similarity_distribution.device)
                    attribute_relation_loss=F.kl_div(proxy_pred_log_softmax, similarity_distribution, reduction='batchmean')
                    
                    

                    loss+=cfg.LOSS_RATIO.L_ACL*augmented_class_loss*((1 - math.cos(cur_epoch / cfg.SOLVER.MAX_EPOCH * math.pi)) / 2)+cfg.LOSS_RATIO.L_ARL*attribute_relation_loss
...
def train(cfg):
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        attribute_feature_dir="/data/usr/OV-HAR/attribute_end_to_end/FROSTER-main/attribute_features/"
        if not os.path.exists(attribute_feature_dir):
            os.makedirs(attribute_feature_dir)
            
        # model.save_features(os.path.join(attribute_feature_dir,"top_k_videos_features_epoch_"+str(cur_epoch)+".pkl"))
