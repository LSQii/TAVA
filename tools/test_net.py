...
import sys
sys.path.insert(0, "/root/TAVA")
sys.path.insert(0, "/root/TAVA/pytorchvideo-main")
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.MODEL.RECORD_ROUTING:
                preds, routing_state = model(inputs)
                 # 新增：提取preds中的预测概率Tensor（preds是tuple，取最后一个元素）
                ...
                # 新增：提取preds和raw_preds中的预测概率Tensor（都是tuple，取最后一个元素）
                if isinstance(preds, tuple):
                    preds = preds[-1]
                ...
                preds = model(inputs)
                # 处理模型返回的不同格式（当启用 KEEP_RAW_MODEL 和 RAW_MODEL_DISTILLATION 时）
                if isinstance(preds, tuple):
                    # 如果返回的是 tuple，可能是 ([pred, None], [None, None]) 格式
                    if len(preds) == 2 and isinstance(preds[0], (list, tuple)):
                        # 格式: ([pred, None], [None, None]) 或类似格式
                        # 取第一个 list 的第一个非 None 的 Tensor
                        first_list = preds[0]
                        if isinstance(first_list, (list, tuple)) and len(first_list) > 0:
                            # 找到第一个非 None 的 Tensor
                            for item in first_list:
                                ...
                                # 如果都是 None，尝试取第一个元素
                                preds = first_list[0] if len(first_list) > 0 else preds[0]
                        else:
                            preds = preds[0]
                    else:
                        # 格式: (pred, ...) 或其他格式
                    ...
                    # 如果返回的是 list，取第一个非 None 的 Tensor
                    for item in preds:
                        if item is not None and isinstance(item, torch.Tensor):
                            preds = item
                            break
                ...
                if not isinstance(preds, torch.Tensor):
                    raise TypeError(f"preds should be a Tensor, but got {type(preds)}: {preds}")
                
                # 确保 preds 是 Tensor
                if not isinstance(preds, torch.Tensor):
                    raise TypeError(f"preds should be a Tensor, but got {type(preds)}")
